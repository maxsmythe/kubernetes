...
var map_MatchResources = map[string]string{
	...
	"objectName":            "ObjectName specifies the object name which the admission control policy should validate on.",
	"excludeObjectName":     "ExcludeObjectName specifies the object name which the admission control policy should not validate on.",
...
var map_ValidatingAdmissionPolicyBindingSpec = map[string]string{
	...
	"policyName":     "PolicyName references a ValidatingAdmissionPolicy name which the policyBinding binds to. if the resource referred to by PolicyName does not exist, this binding is considered mis-configured and will be ignored Required.",
	"paramName":      "ParamName specifies the parameter resource used to configure the admission control policy. It should point to a Custom Resource which is created out of the CRD specified in ParamSource of ValidatingAdmissionPolicy it binded. If the resource referred to by ParamName does not exist, this binding is considered mis-configured and the FailurePolicy of the policy definition applied.",
...
var map_ValidatingAdmissionPolicySpec = map[string]string{
	...
	"failurePolicy":    "FailurePolicy defines how to handle failures for the admission policy. Failures can occur from invalid policy definitions or bindings, as well as validation errors from CEL expressions. A policy is invalid if spec.paramSource refers to a non-existent Kind. A binding is invalid if spec.paramName refers to a non-existent custom resource.",
}
...
var map_Validation = map[string]string{
	...
	"reason":     "Reason represents a machine-readable description of why this validation failed. If this is the first validation in the list to fail, this reason, as well as the corresponding HTTP response code, are used in the HTTP response to the client. If not set, StatusReasonInvalid is used in the response to the client.",
}
